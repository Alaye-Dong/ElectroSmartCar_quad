...
int obstacle_flag = 0;
int obstacle_number = 0;	//障碍计数
int obstacle1_type = 1, obstacle2_type = -1, obstacle3_type = 0;//若启用Obstacle_Deal，由obstacle_type决定的障碍处理，-1为坡道，1为路障
...
{                   	
	...
	if(e_r2<1000&&g_ValueOfAD[2]>=80&&g_ValueOfAD[3]>=80&&abs(g_ValueOfAD[5]-g_ValueOfAD[0])<=15)
	{
		flag_r_hd = 1;
		BEEP = 1;
	}
  ...
  if(obstacle_flag==0 && obstacle_number<1 && (dl1a_distance_mm<=850&&g_ValueOfAD[2]>=40&&g_ValueOfAD[3]>=40&&dl1a_distance_mm>=300&&abs(v_angle)<=12))
	{
		obstacle_flag = 1;
	}
...
void Huan_Dao_Z()
{
	...
	if(roundcount2<=500&&roundcount2>=0) //小圆小大圆大
	{
	...
	else if(roundcount2>500)//&&roundcount2<=25000&&flag_c_hd==0)
	{
		mpu660_angle();	
...
void Huan_Dao_R()
{
	...
	roundcount =(ctimer_count_read(CTIM0_P34)+ctimer_count_read(CTIM3_P04))*0.1;
	roundcount2 = roundcount2 + roundcount;
	if(roundcount2<=450&&roundcount2>=0)
	{
		cha_err1 = 0.2*g_fDirectionError[2] * g_dirControl_P + (g_fDirectionError[2] - last_err1) * g_dirControl_D*0.4;
		last_err1 = g_fDirectionError[2];
	}
	else if(roundcount2>450)//&&roundcount2<=25000&&flag_c_hd==0
	{
		mpu660_angle() ;	
...
void Obstacle_Deal()
{
	if(obstacle_flag == 1 && obstacle_number == 0)
	{
		if (obstacle1_type == -1)
		{
			Slope();
...
void Obstacle()
{
	...
	switch(obstacle_flag)
	{
		case 1:
				...
				break;
		case 2:	
				Target_speed1 = 50 - cha_err1*0.12; 
				Target_speed2 = 50 + cha_err1*0.12;
				cha_err1 = -50;
				mpu660_angle();	
