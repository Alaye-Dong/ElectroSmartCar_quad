...
void Keystroke_Menu(void)
{
    switch(Dispay_Codename)
    {
				...
				case 26:
					...
					break;
				case 27:
					Keystroke_Menu_TWO_Seven();
					break;
				case 28:
					Keystroke_Menu_TWO_Eight();
					break;
...
//		lcd_showstr(1*8,6,"obsta1_type");
//		lcd_showstr(1*8,7,"obsta2_type");
//		lcd_showstr(1*8,8,"obsta3_type");
		
		if(round1_type == -1) 
		{
			lcd_showstr(12*8,1,"L");
		}
		else
		{
			lcd_showstr(12*8,1,"R");
		}
		if(round2_type == -1) 
		{
			lcd_showstr(12*8,2,"L");
		}
		else
		{
			lcd_showstr(12*8,2,"R");
		}
		if(round3_type == -1) 
		{
			lcd_showstr(12*8,3,"L");
		}
		else
		{
			lcd_showstr(12*8,3,"R");
		}
		if(round4_type == -1) 
		{
			lcd_showstr(12*8,4,"L");
		}
		else
		{
			lcd_showstr(12*8,4,"R");
		}
...
//		lcd_showint32(14*8,6,obstacle1_type,1);
//		lcd_showint32(14*8,7,obstacle2_type,1);
//		lcd_showint32(14*8,8,obstacle3_type,1);
...
{
		This_Codename = 26;			//本页面编号
		Menu_TWO_Display(6);
		//Keystroke_Special_Value(&obstacle1_type);
}
void Keystroke_Menu_TWO_Seven(void)  //27
{
		This_Codename = 27;			//本页面编号
		Menu_TWO_Display(7);
		//Keystroke_Special_Value(&obstacle2_type);
}
void Keystroke_Menu_TWO_Eight(void)  //28
{
		This_Codename = 28;			//本页面编号
		Menu_TWO_Display(8);
...
//		lcd_showfloat(11*8,2,ZX_kd,2,2);
		lcd_showfloat(9*8,3,g_dirControl_P,2,3);
		lcd_showfloat(9*8,4,g_dirControl_D,2,3);
...
void Keystroke_Menu_FOUR(void) //4
{
		...
		lcd_showfloat(5,0,g_ValueOfAD[0],4,2);//左竖		
		lcd_showfloat(65,0,g_ValueOfAD[5],4,2);   //右竖	
		lcd_showfloat(5,1,g_ValueOfAD[3],4,2);  //左横	
		lcd_showfloat(65,1,g_ValueOfAD[2],4,2);  //右横
		lcd_showfloat(5,2,g_ValueOfAD[4],4,2);//右斜			  
		lcd_showfloat(65,2,g_ValueOfAD[1],4,2);  //左斜 
		lcd_showstr(1*8,3,"|05   -32   /41"); 
		
		lcd_showfloat(5,4,Target_speed1,4,0);
		lcd_showfloat(65,4,Target_speed2,4,0);
		lcd_showfloat(5,5,servo,3,0);
		lcd_showfloat(65,5,roundcount2,5,0);
		
		lcd_showfloat(5,6,v_angle,4,3); 
		lcd_showfloat(65,6,angle,4,3); 


		lcd_showfloat(5,7,y_v_angle,4,3); 
		//lcd_showfloat(65,7,angle_y,4,3);
		lcd_showfloat(65,7,roundcount2_k,4,0);
		
		lcd_showfloat(5,8,e_r1,5,0);
		lcd_showfloat(65,8,e_r2,5,0);
...
void Menu_FIVE_Display(uint8 control_line)
{
		...
		lcd_showstr(1*8,7,"gErr2"); 
		lcd_showfloat(65,7,g_fDirectionError[2],4,3);
		lcd_showstr(1*8,8,"chaerr1"); 
		lcd_showfloat(65,8,cha_err1,4,3);
		lcd_showstr(1*8,9,"servo"); 
		lcd_showfloat(65,9,servo,4,3);
